...
for attempt in $(seq 1 30); do
  if docker_compose exec -T app python -c "import urllib.request; print(urllib.request.urlopen('http://127.0.0.1:8000/healthz',timeout=4).read().decode())" >/dev/null 2>&1; then
